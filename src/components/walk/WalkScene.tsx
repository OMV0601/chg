  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const render = (p: number) => {
        ...
        const push = smoothstep(range(p, WALK_END, 0.82));
        if (world) {
          world.style.transform = `scale(${1 + push * 5.4})`;
          world.style.opacity = String(1 - range(p, 0.72, 0.83));
        }

        // Inside: the room settles and the machines come on one by one.
        const inside = range(p, 0.71, 0.84);
        if (lab) {
          lab.style.opacity = String(inside);
          lab.style.transform = `scale(${1.28 - smoothstep(inside) * 0.28})`;
        }
        screens.forEach((s, i) => {
          const at = 0.775 + i * 0.011;
          s.style.opacity = String(range(p, at, at + 0.02) * 0.92);
        });

        // Then a real photograph of a real lab, if we have one.
        fade(photo, range(p, 0.87, 0.95));
        ...
        const heroIn = smoothstep(range(p, 0.88, 0.99));
        if (hero) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          {labPhoto && (
            <div data-lab-photo className="absolute inset-0" style={{ opacity: 0 }}>
              ...
              />
              <div className="absolute inset-0 bg-peacock-deep/45" />
            </div>
          )}
        </div>
        ...
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center px-6">
          {beats.map((beat) => (
            <p
              key={beat.text}
              data-beat
              className="absolute max-w-[22ch] text-center font-display text-h2 font-semibold text-whitewash drop-shadow-[0_2px_24px_rgba(8,20,30,0.85)]"
              style={{ opacity: 0 }}
            >
              {beat.text}
            </p>
        ...
        >
          <h1 className="font-display text-display font-bold text-whitewash drop-shadow-[0_2px_30px_rgba(8,20,30,0.9)]">
            {org.name}
          </h1>
        ...
        <div
          data-cue
          className="pointer-events-none absolute inset-x-0 bottom-8 text-center font-display text-small text-whitewash/80"
        >
